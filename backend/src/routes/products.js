...
router.get('/', async (req, res) => {
  ...
  try {
    const countRes = await pool.query(
      `SELECT COUNT(*) FROM products ${where}`, values
    );
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      ...
      [req.body.image1, req.body.image2, req.body.image3, req.body.image4, req.body.image5,
       name, price, original_price, stock, status, is_new, is_sold_out, description, featured_order, req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Product not found' });
    res.json(rows[0]);
  } catch (err) {
