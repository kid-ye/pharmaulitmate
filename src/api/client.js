...
const userRequest = async (path, options = {}) => {
  const res = await fetch(`/api${path}`, { headers: userHeaders(), ...options });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Request failed');
  return data;
};
export const getProfile      = () => userRequest('/users/profile');
export const updateProfile   = (body) => userRequest('/users/profile', { method: 'PUT', body: JSON.stringify(body) });
export const changePassword  = (body) => userRequest('/users/password', { method: 'PUT', body: JSON.stringify(body) });
...
export const addAddress      = (body) => userRequest('/users/addresses', { method: 'POST', body: JSON.stringify(body) });
export const updateAddress   = (id, body) => userRequest(`/users/addresses/${id}`, { method: 'PUT', body: JSON.stringify(body) });
export const deleteAddress   = (id) => userRequest(`/users/addresses/${id}`, { method: 'DELETE' });
...
export const uploadProductImage  = async (files) => {
  ...
  const res = await fetch('/api/products/upload', {
    method: 'POST',
    headers: { Authorization: `Bearer ${localStorage.getItem('pharma-admin-token')}` },
    body: form,
  });
  const data = await res.json();
...
export const getOrders       = (params = {}) => { const qs = new URLSearchParams(params).toString(); return request(`/orders${qs ? `?${qs}` : ''}`); };
...
// Reviews
export const getReviews    = (productId) => request(`/reviews${productId ? `?productId=${productId}` : ''}`);
export const submitReview  = (body) => request('/reviews', { method: 'POST', body: JSON.stringify(body) });
export const featureReview = (id) => request(`/reviews/${id}/feature`, { method: 'PATCH' });

// Contact
export const sendContact = (body) => request('/contact', { method: 'POST', body: JSON.stringify(body) });
...
// Cart
export const loadCart  = (email) => request(`/cart?email=${encodeURIComponent(email)}`);
export const syncCart  = (email, items) => request('/cart/sync', { method: 'POST', body: JSON.stringify({ email, items }) });

// Wishlist
export const getWishlist      = () => request('/wishlist');
export const addToWishlist    = (productId) => request(`/wishlist/${productId}`, { method: 'POST' });
