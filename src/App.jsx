...
function FadeUpObserver() {
  const { pathname } = useLocation();
  useEffect(() => {
    // Adding a short timeout ensures the DOM has fully rendered the dynamic content
    // before the observer tries to attach to it.
...
function App() {
  ...
  const [user, setUser] = useState(() => {
    try {
      const u =
        localStorage.getItem("pharma-user") ||
        sessionStorage.getItem("pharma-user");
      const t =
        localStorage.getItem("pharma-user-token") ||
        sessionStorage.getItem("pharma-user-token");
      if (u && t) {
  ...
  const handleAuth = async (u) => {
    ...
    let dbCart = [];
    try {
      dbCart = (await loadCart(u.email)).map((r) => ({
        id: r.product_id,
        name: r.name,
        category: r.category,
          ...
          element={
            ...
            >
              <Routes>
                <Route
                  path="/signin"
                  element={<SignIn onAuth={handleAuth} />}
                />
                <Route
                  path="/profile"
                  element={
